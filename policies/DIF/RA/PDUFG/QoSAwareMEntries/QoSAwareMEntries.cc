...
#include "DIF/RA/PDUFG/QoSAwareMEntries/QoSAwareMEntries.h"
...
void QoSAwareMEntries::insertedFlow(const Address &addr, const QoSCube &qos, RMTPort * port){
...
void QoSAwareMEntries::routingUpdated(){
...
void QoSAwareMEntries::onPolicyInit(){
    ...
    for(auto link : Links){
        ...
        if(src == myAddr) { dstLat[dst] = lat; }
        else if(dst == myAddr) { dstLat[src] = lat; }
    }
}
