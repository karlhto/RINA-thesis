...
void PLQoSAwareMEntries::insertedFlow(const Address &addr, const QoSCube &qos, RMTPort * port){
...
void PLQoSAwareMEntries::routingUpdated(){
    ...
    for(const auto & qosEntries : changes){
        ...
        for(const auto & entry : qosEntries.second){
            ...
            if(ps.empty()) {
                for(auto qId : qIds) {
                    qId->setDistance(entry.first, qosId, INT_MAX);
                }
            } else {
                for(auto qId : qIds) {
                    qId->setDistance(entry.first, qosId, entry.second.metric);
                }
            }
        }
    }
...
void PLQoSAwareMEntries::onPolicyInit(){
...
void PLQoSAwareMEntries::registerQidsGen(IDPerNQoSxPLen * qId) {
    qIds.insert(qId);
}
void PLQoSAwareMEntries::unregisterQidsGen(IDPerNQoSxPLen * qId) {
    qIds.erase(qId);
}
