...
void MM_DQ_Drop::initialize() {
    ...
    defaultThreshold = par("defaultThreshold").intValue();
...
MM_DQ_Drop::~MM_DQ_Drop(){
    portCount.clear();
}

void MM_DQ_Drop::pduInsertered(RMTQueue * q, RMTPort * p) {
    portCount[p]++;
}

void MM_DQ_Drop::pduDropped(RMTQueue * q, const cPacket * s, RMTPort * p) {
    portCount[p]--;
}

void MM_DQ_Drop::pduReleased(RMTQueue * q, RMTPort * p) {
    portCount[p]--;
}

void MM_DQ_Drop::queueCreated(RMTQueue * q, RMTPort * p) {
...
double MM_DQ_Drop::getDropProbability(RMTQueue * q, RMTPort * p) {
